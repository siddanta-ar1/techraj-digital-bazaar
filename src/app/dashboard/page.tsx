...
export default async function DashboardPage() {
  ...
  const [{ data: userProfile }, { data: orders }] = await Promise.all([
    admin.from("users").select("full_name, wallet_balance, email").eq("id", user.id).single(),
    admin.from("orders").select("status").eq("user_id", user.id),
  ]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-all group">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 mt-1">{completedOrders}</p>
              </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Link href="/dashboard/orders" className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg hover:border-indigo-200 transition-all group relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <h3 className="font-bold text-slate-900 text-lg mb-1 group-hover:text-indigo-600 transition-colors">My Orders</h3>
                <p className="text-sm text-slate-500 mb-4">View your purchase history and track current orders.</p>
            ...
            <Link href="/dashboard/wallet" className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg hover:border-indigo-200 transition-all group relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              ...
              <div className="relative z-10">
                ...
                <h3 className="font-bold text-slate-900 text-lg mb-1 group-hover:text-indigo-600 transition-colors">Wallet Top-up</h3>
                <p className="text-sm text-slate-500 mb-4">Add funds to your wallet for quick checkouts.</p>
                <div className="flex items-center text-sm font-medium text-indigo-600 group-hover:translate-x-1 transition-transform">
            ...
            <Link href="/dashboard/settings" className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg hover:border-indigo-200 transition-all group relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              ...
              <div className="relative z-10">
                ...
                <h3 className="font-bold text-slate-900 text-lg mb-1 group-hover:text-indigo-600 transition-colors">Settings</h3>
                <p className="text-sm text-slate-500 mb-4">Update your profile details and preferences.</p>
                <div className="flex items-center text-sm font-medium text-indigo-600 group-hover:translate-x-1 transition-transform">

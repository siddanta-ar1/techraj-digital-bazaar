...
export default function OrderDetailsClient({ order }: OrderDetailsClientProps) {
  ...
  const getPaymentIcon = () => {
    switch (order.payment_method) {
      case 'wallet': return <CreditCard className="h-5 w-5 text-indigo-600" />
      case 'esewa': return <div className="h-5 w-5 bg-blue-500 rounded"></div>
      case 'bank_transfer': return <div className="h-5 w-5 bg-purple-500 rounded"></div>
      default: return null
    }
  }
  ...
  const deliveredItems = order.order_items.filter(item => item.delivered_code)
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-8">
          ...
          <div className="relative">
            ...
            {[
              ...
              { 
                ...
                time: order.payment_status === 'paid' ? 'Payment successful' : 'Pending payment',
                icon: CreditCard
              },
              { 
                status: 'Processing', 
                active: order.status === 'processing' || order.status === 'completed',
                time: order.status === 'processing' ? 'Currently processing' : 'Processed',
                icon: Clock
              },
              { 
                status: order.delivery_type === 'auto' ? 'Auto Delivery' : 'Manual Delivery', 
                active: order.status === 'completed' || deliveredItems.length > 0,
                time: deliveredItems.length > 0 ? `${deliveredItems.length} item${deliveredItems.length !== 1 ? 's' : ''} delivered` : 'Pending',
                icon: Truck
              },
              { 
                status: 'Completed', 
                active: order.status === 'completed',
                time: order.status === 'completed' ? 'Order completed' : 'Not completed',
                icon: CheckCircle
              }
            ].map((step, index) => {
        ...
        {/* Order Items */}
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-lg font-semibold text-slate-900 mb-6">Order Items</h2>
          <div className="space-y-4">
            {order.order_items.map((item) => (
              <div key={item.id} className="flex items-center gap-4 p-4 border rounded-lg">
                <div className="relative w-16 h-16 flex-shrink-0">
                  <Image
                    src={item.variant.product.featured_image || '/placeholder-product.jpg'}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-600">{item.variant.variant_name}</p>
                    ...
                    <div className="mt-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 bg-slate-100 px-3 py-1 rounded flex items-center justify-between">
                          <code className="text-sm font-mono">{item.delivered_code}</code>
                          <button
          ...
          <div className="mt-8 pt-8 border-t">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between pt-3 border-t">
                ...
                <span className="text-lg font-bold text-slate-900">Rs. {total.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="font-medium text-slate-900">{order.user?.full_name || 'Not provided'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium text-slate-900">{order.user?.phone || 'Not provided'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="space-y-3">
            {order.payment_method === 'bank_transfer' && order.payment_status === 'pending' && (
              <button className="w-full flex items-center justify-center gap-2 bg-amber-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-amber-700">

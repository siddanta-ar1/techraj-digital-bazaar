...
import { Menu, Search, ShoppingCart, User, Wallet, LogOut, ChevronDown } from 'lucide-react'
      ...
      <div className="container mx-auto px-4 py-3 md:py-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-6 lg:gap-8">
            <Link href="/" className="text-sm font-semibold text-slate-600 hover:text-indigo-600 transition-colors">Home</Link>
            <Link href="/products" className="text-sm font-semibold text-slate-600 hover:text-indigo-600 transition-colors">Products</Link>
            <Link href="/category/games" className="text-sm font-semibold text-slate-600 hover:text-indigo-600 transition-colors">Game Codes</Link>
            <Link href="/category/gift-cards" className="text-sm font-semibold text-slate-600 hover:text-indigo-600 transition-colors">Gift Cards</Link>
          </nav>

          {/* Search and Actions */}
          <div className="flex items-center gap-3">
            {/* Search Bar */}
            <div className="hidden md:flex items-center bg-slate-100 rounded-full px-4 py-2 border border-transparent focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-100 focus-within:bg-white transition-all w-64">
              <Search className="w-4 h-4 text-slate-400" />
              <input 
                type="text" 
                placeholder="Search products..." 
                className="bg-transparent ml-2 outline-none w-full text-sm text-slate-700 placeholder-slate-400"
              />
            </div>

            {/* Cart */}
            <Link href="/cart" className="p-2.5 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 rounded-full relative transition-colors">
            ...
            {user ? (
                ...
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl shadow-slate-200/50 border border-slate-100 py-2 z-50 animate-in fade-in zoom-in-95 duration-100">
                    ...
                    >
                      ...
                      Wallet <span className="ml-auto text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">रु {user.wallet_balance.toFixed(0)}</span>
                    </Link>
              ...
              <Link href="/login" className="hidden md:flex bg-indigo-600 text-white px-5 py-2 rounded-full text-sm font-semibold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-600/20">
                Sign In
              </Link>
            )}
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-4 pb-4 border-t border-slate-100 pt-4 animate-in slide-in-from-top-2">
            <div className="flex flex-col gap-1">
              <Link href="/" className="py-3 px-4 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg font-medium">Home</Link>
              <Link href="/products" className="py-3 px-4 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg font-medium">Products</Link>
              <Link href="/category/games" className="py-3 px-4 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg font-medium">Game Codes</Link>
              <Link href="/dashboard" className="py-3 px-4 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg font-medium">Dashboard</Link>
              
              {!user && (
                 <Link href="/login" className="mt-4 mx-4 py-3 bg-indigo-600 text-white rounded-lg text-center font-bold">
                  Sign In
                </Link>
              )}
